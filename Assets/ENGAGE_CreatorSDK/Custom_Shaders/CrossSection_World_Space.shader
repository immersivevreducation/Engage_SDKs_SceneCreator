Shader "CrossSection/CrossSection_World_Space"{
    ...
    SubShader{
        ...
        void surf (Input i, inout SurfaceOutputStandard o) {
            ...
            float dist = 1 * (i.worldPos - _Plane.xyz);
            distance = distance + _Plane.w;
            //discard surface above plane
            clip(-distance);

            float facing = i.facing * 0.5 + 0.5;
            //fixed4 _Cutoffsurface = tex2D(_CutOffTex, i.uv_CutOffTex) *_CutoffColor;
            //normal color stuff
            fixed4 col = lerp(tex2D(_MainTex, i.uv_MainTex),_CutoffColor,  clamp(dot(i.worldPos, _Plane.xyz) - _cutoffMask,0,1));        
